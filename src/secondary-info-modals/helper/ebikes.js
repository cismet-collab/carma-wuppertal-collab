export function convertVTEntryToFeatureProperties(vt) {
  ...
  let adresse =
    vt.adresse && typeof vt.adresse === "string" && vt.adresse.trim()
      ? vt.adresse.trim()
      : undefined;
  ...
  if (
    vt.gruen &&
    typeof vt.gruen === "string" &&
    vt.gruen.trim() &&
    vt.gruen.startsWith("http")
  ) {
    gruener_strom = true;
  }
  ...
  const clone = JSON.parse(JSON.stringify(vt));
  ...
  const properties = {
    ...
    ladebox_pfand: pfandArr.length
      ? pfandArr.map((s) => s.replace("€", ""))
      : undefined,
    pfand: pfandArr.length
      ? pfandArr.map((s) => s.replace("€", ""))
      : undefined,
    oeffnungszeiten:
      vt.oeffnungszeiten ??
